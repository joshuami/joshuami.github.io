...
class WebformActions {
  ...
  public function formActionChangeEmails($data) {
    ...
    if (!empty($data)) {
      ...
      return "We changed how the e-mail is set.";
    }
    return "Could change the url, maybe it was not valid?";
  }
        ...
        foreach ($data as $ask) {
          switch ($ask['action']) {
            ...
            case 'emails':
              ...
              if (count($email)) {
                $response .= '<p>The form will send an email to the following e-mails or fields: <strong>' . implode(", ", $email) . '</strong>.</p>';
              }
              else {
                $response .= '<p>The form will not send an e-mail to anyone.</p>';
              }
              break;
          }
        }
  ...
  public function generateFormField(array $field) {
    ...
    if ($fieldType) {
      ...
      $newField = $this->getFormatedElement($newField);
   ...
   * Special cases.
   */
  public function getFormatedElement($formatted) {
  ...
  public function getPossibleFields() {
    ...
    $webformStorage = \Drupal::service('plugin.manager.webform.element');
    return $webformStorage->getDefinitions();
  }
  ...
  public function getPossibleFieldNames() {
    ...
    $webformStorage = \Drupal::service('plugin.manager.webform.element');
    $names = [];
    foreach ($webformStorage->getDefinitions() as $definition) {
