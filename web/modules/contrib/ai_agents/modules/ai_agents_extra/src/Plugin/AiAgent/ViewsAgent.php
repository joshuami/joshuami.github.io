...
use Drupal\wse_config\WseConfigWorkspaceManager;
...
class ViewsAgent extends AiAgentBase {
  ...
  public function loadWorkspace() {
    ...
    $workspace = \Drupal::entityTypeManager()
      ...
      ->loadByProperties(['label' => $workspace_name]);

    if (!count($workspace)) {
      throw new \Exception('Could not load workspace.');
    }
    $workspaceManager = \Drupal::service('workspaces.manager');
    $workspaceManager->setActiveWorkspace($workspace[key($workspace)]);
  }
  ...
  public function determineTypeOfTask() {
    ...
    throw new \Exception('Invalid action in determing the task.');
  }
  ...
  public function getFieldConfigs($fieldType) {
    $typedViews = \Drupal::service('config.typed');
  ...
  public function getFieldTypes() {
    $fields = \Drupal::service('entity_field.manager')->getFieldDefinitions($this->data[0]['entity_type'], $this->data[0]['bundle']);
    $list = [];
  ...
  public function getFilterPluginOptions() {
    $filterViews = \Drupal::service('plugin.manager.views.filter');
    $typedViews = \Drupal::service('config.typed');

    $filters = $filterViews->getDefinitions();
    $list = [];
    foreach ($filters as $filter) {
      $typed = $typedViews->getDefinition('views.filter.' . $filter['id']);
      $list[] = $filter['id'];
    }
    return implode("\n", $list);
  }
  ...
  public function getFieldPluginOptions() {
    $fieldViews = \Drupal::service('plugin.manager.views.field');
    $typedViews = \Drupal::service('config.typed');

    $fields = $fieldViews->getDefinitions();
    $list = [];
    foreach ($fields as $field) {
      $typed = $typedViews->getDefinition('views.field.' . $field['id']);
  ...
  public function getViewsTypes() {
    $formats = \Drupal::service('plugin.manager.views.display')->getDefinitions();
    $list = [];
  ...
  public function getViewsStyles() {
    $formats = \Drupal::service('plugin.manager.views.style')->getDefinitions();
    $list = [];
    foreach ($formats as $plugin_definition) {
      $list[] = $plugin_definition['id'];
    }
  ...
  public function getEntityTypesAndBundles() {
    $nodeTypes = \Drupal::service('entity_type.bundle.info')->getAllBundleInfo();
    $list = "";
  ...
  public function getStorageTable($fieldName, $entityType, $revision = FALSE) {
    ...
    $type = $entityTypeManager->getDefinition($entityType);
    ...
    if ($type instanceof ContentEntityTypeInterface && in_array(SqlEntityStorageInterface::class, class_implements($type->getStorageClass()))) {
      $storage = $entityTypeManager->getStorage($type->id());

      foreach ($fieldManager->getFieldStorageDefinitions($type->id()) as $field) {
