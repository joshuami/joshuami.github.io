...
class FieldType extends AiAgentBase implements ContainerFactoryPluginInterface {
  ...
  public function __construct(
    ...
    parent::__construct($configuration, $plugin_id, $plugin_definition, $agentHelper, $fileSystem, $configFactory, $currentUser, $extensionPathResolver, $promptJsonDecoder, $providerManager);
  }
  ...
  public function answerQuestion() {
    ...
    $context = [
      'Installed Field Types List' => $this->fieldAgentHelper->getFieldTypesList(),
      'Entity Types/Bundles List' => $this->fieldAgentHelper->getEntityTypesAndBundles(),
    ];
    if ($this->answerContext['label'] && $this->answerContext['entity_type'] && $this->answerContext['bundle_name']) {
      // Field has to exist.
      ...
      if ($fieldStorage) {
        $context['Storage Settings for ' . $fieldName] = Yaml::dump($fieldStorage->get('settings'));
      }
      $fieldConfig = $this->fieldAgentHelper->fieldConfigExists($this->answerContext);
      if ($fieldConfig) {
        $context['Field Config Settings for ' . $fieldName] = Yaml::dump($fieldConfig->get('settings'));
      }
      $context['Form Widget Settings ' . $fieldName] = Yaml::dump($this->fieldAgentHelper->getFormDisplay($this->answerContext));
      $context['Display Widget Settings ' . $fieldName] = Yaml::dump($this->fieldAgentHelper->getViewDisplay($this->answerContext));
    }
  ...
  public function storeFieldConfigurations(array $dataPoint) {
    ...
    $config = \Drupal::entityTypeManager()->getStorage('field_config')->load($configId);
    ...
    if (in_array($fieldType, ['entity_reference', 'entity_reference_revisions'])) {
      ...
      $storage = \Drupal::entityTypeManager()->getStorage('field_storage_config')->load($storageId);
    ...
    $display = \Drupal::service('entity_display.repository')->getViewDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], 'default');
    ...
    $display = \Drupal::service('entity_display.repository')->getFormDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], 'default');
    $component = $display->getComponent($dataPoint['field_name']);
    if ($component) {
  ...
  public function storeDisplayMode(array $dataPoint, array $data) {
    if (isset($data[0]['display_view_mode']) && $data[0]['display_view_mode'] != 'default') {
      $display = \Drupal::service('entity_display.repository')->getViewDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], $data[0]['display_view_mode']);
      $component = $display->getComponent($dataPoint['field_name']);
    }
    else {
      $display = \Drupal::service('entity_display.repository')->getViewDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], 'default');
  ...
  public function reorderFieldDisplay(array $dataPoint) {
    ...
    $view_display = \Drupal::service('entity_display.repository')->getViewDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], 'default');
  ...
  public function reorderFieldForm(array $dataPoint) {
    ...
    $form_display = \Drupal::service('entity_display.repository')->getFormDisplay($dataPoint['entity_type'], $dataPoint['bundle_name'], 'default');
    $this->setOriginalConfigurations($form_display);
    // Try to reshuffle.
  ...
  public function reshuffleComponents(EntityFormDisplayInterface|EntityViewDisplayInterface $display) {
    ...
    foreach ($components as $component => $data) {
      ...
      if (($field_definition && $field_definition->isDisplayConfigurable($type)) || $data['type'] == 'extra_field') {
