...
(function($,Drupal){Drupal.behaviors.menuUiChangeParentItems={attach(context,settings){const menu=once('menu-parent','#edit-menu');if(menu.length){const $menu=$(menu);Drupal.menuUiUpdateParentList();$menu.on('change','input',Drupal.menuUiUpdateParentList);}}};Drupal.menuUiUpdateParentList=function(){const $menu=$('#edit-menu');const values=[];$menu.find('input:checked').each(function(){values.push(Drupal.checkPlain(this.value));});$.ajax({url:`${window.location.protocol}//${window.location.host}${Drupal.url('admin/structure/menu/parents')}`,type:'POST',data:{'menus[]':values},dataType:'json',success(options){const $select=$('#edit-menu-parent');const selected=$select[0].value;$select.children().remove();let totalOptions=0;Object.keys(options||{}).forEach((machineName)=>{const selectContents=document.createElement('option');selectContents.selected=machineName===selected;selectContents.value=machineName;selectContents.textContent=options[machineName];$select.append(selectContents);totalOptions++;});$select.closest('div').toggle(totalOptions>0).attr('hidden',totalOptions===0);}});};})(jQuery,Drupal);;
...
(function($,Drupal){Drupal.behaviors.contentTypes={attach(context){const $context=$(context);$context.find('#edit-submission').drupalSetSummary((context)=>{const values=[];values.push(Drupal.checkPlain($(context).find('#edit-title-label')[0].value)||Drupal.t('Requires a title'));return values.join(', ');});$context.find('#edit-workflow').drupalSetSummary((context)=>{const values=[];$(context).find('input[name^="options"]:checked').next('label').each(function(){values.push(Drupal.checkPlain(this.textContent));});if($(context).find('#edit-options-status:checked').length===0)values.unshift(Drupal.t('Not published'));return values.join(', ');});$('#edit-language',context).drupalSetSummary((context)=>{const values=[];values.push($('.js-form-item-language-configuration-langcode select option:selected',context)[0].textContent);$('input:checked',context).next('label').each(function(){values.push(Drupal.checkPlain(this.textContent));});return values.join(', ');});$context.find('#edit-display').drupalSetSummary((context)=>{const values=[];const $editContext=$(context);$editContext.find('input:checked').next('label').each(function(){values.push(Drupal.checkPlain(this.textContent));});if($editContext.find('#edit-display-submitted:checked').length===0)values.unshift(Drupal.t("Don't display post information"));return values.join(', ');});}};})(jQuery,Drupal);;
(($,Drupal)=>{Drupal.behaviors.simpleSitemapFieldsetSummaries={attach(context){$(context).find('.simple-sitemap-fieldset').drupalSetSummary((fieldset)=>{let summary='';const enabledVariants=[];$(fieldset).find('input:checkbox[name*="simple_sitemap_index_now"]').each(function each(){summary=`${this.checked?Drupal.t('IndexNow notification enabled'):Drupal.t('IndexNow notification disabled')}, `;});$(fieldset).find('input:radio:checked[data-simple-sitemap-label][value="1"]').each(function each(){enabledVariants.push(this.dataset.simpleSitemapLabel);});if(enabledVariants.length>0){summary+=Drupal.t('Included in sitemaps: ');summary+=enabledVariants.join(', ');}else summary+=Drupal.t('Excluded from all sitemaps');return summary;});}};})(jQuery,Drupal);;
